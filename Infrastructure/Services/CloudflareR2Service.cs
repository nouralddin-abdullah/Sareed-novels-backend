...
public class CloudflareR2Service(IAmazonS3 s3Client, IOptions<CloudflareR2Settings> settings) : IFileUploadService
{
    ...
    public async Task<string> UploadReadingListCoverImageAsync(Stream fileStream, string contentType, string readingListId)
    {
        var key = $"reading-list-images/{readingListId}";
        var request = new PutObjectRequest
        {
            BucketName = "sard",
            Key = key,
    ...
    public async Task<string> UploadPostImageAsync(Stream fileStream, string contentType, string postId)
    {
        var key = $"post-images/{postId}";
        var request = new PutObjectRequest
        {
            BucketName = "sard",
            Key = key,
    ...
    public async Task<string> UploadPaymentProofAsync(Stream fileStream, string contentType, string userId)
    {
        var key = $"payment-proofs/{userId}/{Guid.NewGuid()}";
        var request = new PutObjectRequest
        {
            BucketName = "sard",
            Key = key,
